...
void fill_potential_container(float *PotInput,int Ngrid,double boxsize,struct potential_container **PotCont,int Snap){ //Computes all spacial derivatives.
  ...
  kscale=2.0*CONSPI/(boxsize*(double)Ngrid3);
  // if(isinf(kscale)) printf("I know\n");
  ...
  kscale=4.0*CONSPI*CONSPI/(boxsize*boxsize*(double)Ngrid3);
...
int overdens(double *Alldens,int Ngrid){
  long a;
  double sum=0;
  printf("	Rescale density to contrast ... ");	fflush(stdout);
...
double get_potential_value(double posx,double posy,double posz,double gridsize,int Ngrid,struct potential_container **Pot,int Snap){
  ...
  z=abs(posz/gridsize);
  //remove
 // printf("get potential %d\t%f,%d\t%f,%d\t%f\n",x,posx,y,posy,z,posz);
  return Pot[shift_koor(x,y,z,Ngrid)][Snap].val;
}

void get_potential(char *fname,float **pot,int Ngrid,struct header_snapshot Head,int FileNr){
  FILE *fileptr;
  int Ncheck,ZERO_FILE;
  float *PotHold=pot[0];
  double *dens;
  struct particle_data *Pptr;
  if (SNAP_ORDER == 1) ZERO_FILE = run.NSnap-1;
  else ZERO_FILE = 0;
  if ((fileptr=fopen(fname,"r"))) {
    ...
    IOTime[0]+=time(NULL);
    #endif
    //If potential already computed last snapshot is read again to compute observors velocity
    if (FileNr==ZERO_FILE) {
      #ifdef MEASURETIME
      IOTime[0]-=time(NULL);
      #endif
      printf("Reading earliest redshift value snapshot to compute observor's velocity\n" );
      fflush(stdout);
      load_snapshot(FileNr,1,&Pptr,1);
      find_closest_particle(Head.npartTotal[1],&Pptr);
      free(Pptr);
      #ifdef MEASURETIME
      IOTime[0]+=time(NULL);
      #endif
    ...
    fflush(stdout);
    //reads potential incorrectly
    if (FileNr==ZERO_FILE) {load_snapshot(FileNr,1,&Pptr,1);
    find_closest_particle(Head.npartTotal[1],&Pptr);}
    else load_snapshot(FileNr,1,&Pptr,0);
    dens=density_field(Pptr,Head,Ngrid,1);// Calucales n using cic'
    free(Pptr);
    overdens(dens,Ngrid);
    free(PotHold);//Pointer redefinition fixed
    PotHold=grav_potential(&dens,Ngrid,Head);
    pot[0] = PotHold;
    free(dens);//NOt Sure yet
...
void find_closest_particle(int particle_nr,struct particle_data **Pptr){
  float r_dist,min_r_dist=200000000.0,x_per,y_per,z_per;
  int idx_min_r;
  struct particle_data *P;
  if ((run.ObsVel[0]==0.0)&&(run.ObsVel[1]==0.0)&&(run.ObsVel[2]==0.0)){
    printf("Observer velocity set to zero, the code will look for the nearest particle to the observer and take its phase space position.\n");fflush(stdout);
    P=Pptr[0];
    for (int idx_par = 0; idx_par < particle_nr; idx_par++) {
      x_per = P[idx_par].Pos[0]-run.ObsPos[0];
      y_per = P[idx_par].Pos[1]-run.ObsPos[1];
      z_per = P[idx_par].Pos[2]-run.ObsPos[2];
      // if (idx_par<25)
      // printf("Obsevor's specs Particle ID %i:\n Position (%f\t%f\t%f)\nVelocity (%f\t%f\t%f)\n",idx_par,P[idx_par].Pos[2],P[idx_par].Pos[1],P[idx_par].Pos[0],P[idx_par].Vel[0],P[idx_par].Vel[1],P[idx_par].Vel[2]);fflush(stdout);
      // r_dist = sqrt(*(P[idx_par].Pos[0]-obs_pos_x) + (P[idx_par].Pos[1]-obs_pos_y)*(P[idx_par].Pos[1]-obs_pos_y) + (P[idx_par].Pos[2]-obs_pos_z)*(P[idx_par].Pos[2]-obs_pos_z) );
      r_dist = sqrt(x_per*x_per + y_per*y_per + z_per*z_per);
    ...
    // SqrtExpan = sqrt(header1.time);//To get peculiar velocity push to start observor at different redshifts
    run.ObsVel[0] = P[idx_min_r].Vel[0];
    run.ObsVel[1] = P[idx_min_r].Vel[1];
    run.ObsVel[2] = P[idx_min_r].Vel[2];
    run.ObsPos[0] = P[idx_min_r].Pos[0];
    run.ObsPos[1] = P[idx_min_r].Pos[1];
    run.ObsPos[2] = P[idx_min_r].Pos[2];

    printf("Observer's specs Particle ID %i:\n Position (\t%f\t%f\t%f)\nVelocity (\t%f\t%f\t%f)\n",idx_min_r,run.ObsPos[0],run.ObsPos[1],run.ObsPos[2],run.ObsVel[0],run.ObsVel[1],run.ObsVel[2]);
  }
  else{
    printf("Observer's specs set from paramater file are: Particle ID %i:\n Position (\t%f\t%f\t%f)\nVelocity (\t%f\t%f\t%f)\n",idx_min_r,run.ObsPos[0],run.ObsPos[1],run.ObsPos[2],run.ObsVel[0],run.ObsVel[1],run.ObsVel[2]);
  }
  fflush(stdout);
}
...
double PeriDist(double dist,double box){
  ...
  double result;
  if (dist>box/2.) result= dist-box; else if (dist<-box/2.) result= dist+box; else result= dist;
  return result;
}
